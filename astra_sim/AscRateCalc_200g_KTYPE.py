...
m_p = np.array([.25,.5,.75,1])#np.array([0.588, .844])#np.arange(0.5,0.8,.05)#np.array([0.582]) #kg             PAYLOAD MASS
v = np.arange(0,7,.25) #m/s              ASCENT RATE, SWEEP
m_b = 0.2 #kg                           BALLOON MASS
...
R_Burst = 2.4834#(meters)
V_Burst = 4/3 * (np.pi * R_Burst**3)
R_Burst = 1.4935#(96/2) / 39.37 # meters (from in)
R_Nominal = (78/2) / 39.37
V_Burst = 4/3 * (np.pi * R_Burst**3)
...
H2.SetLim(Left = 1, Right = 6, Bottom = 0, Top = 3)
rindex = 0
for myR in Rinits:
