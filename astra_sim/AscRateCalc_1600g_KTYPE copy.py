...
m_p = np.array([.25,.5,.75,1,1.2])#np.array([0.588, .844])#np.arange(0.5,0.8,.05)#np.array([0.582]) #kg             PAYLOAD MASS
v = np.arange(0,7,.25) #m/s              ASCENT RATE, SWEEP
m_b = 1.6 #kg                           BALLOON MASS
...
rindex = 0
H2.Plot((v, 2*Rinits[0]),  Color= Clr)
# for myR in Rinits:
...
VolLim = 115/35.3153 #cuft3 to m3
DLim = 2*((VolLim * (3/ (4*np.pi)))**(1/3))
...
H2.plt.axhline(y= DLim,label = 'Regulation Limit', xmin =0,xmax = maxP, linestyle = '--', color = 'k')
